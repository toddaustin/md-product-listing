...
function addItem() {
    for (var n = 0; n < addToCart.length; n++) {
        addToCart[n].addEventListener('click', function() {
            ...
            var totalPrice = parseFloat(curProdPrice * curProdQuantity).toFixed(2);
    ...
    for (var k = 0; k < cartQty.length; k++) {
        cartQty[k].addEventListener('change', function() {
            ...
            if (curAmount == 0) {
                ...
                var newAmount = parseFloat(curItemPrice * curAmount).toFixed(2);
                updatePrice[0].innerHTML = "$" + newAmount;
                updateCartCount();
            }
        });
    }
